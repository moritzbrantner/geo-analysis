...
import * as wasm from "@moenarch/geo-io-geojson-wasm";

const operationInputs = {
  "geoJson.toGeoJson": {
    geometry: {
      type: "Point",
      coordinates: [8.0, 49.0],
    },
  },
  "geoJson.bounds": {
    geoJson: {
...
const operationOrder: OperationId[] = ["geoJson.toGeoJson", "geoJson.bounds", "describe"];
...
export function App() {
  const [operation, setOperation] = useState<OperationId>("geoJson.bounds");
  const [input, setInput] = useState(formatJson(operationInputs["geoJson.bounds"]));
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      <div className="mx-auto grid max-w-6xl gap-6 px-4 py-6 md:grid-cols-[18rem_1fr]">
        <aside className="space-y-4 border-b border-slate-200 pb-4 md:border-b-0 md:border-r md:pr-6">
          <div>
            <h1 className="text-2xl font-semibold">GeoJSON I/O</h1>
            <p className="mt-1 text-sm text-slate-600">{surface?.library ?? "moenarch-geo-io-geojson"}</p>
          </div>
        ...
        <section className="grid gap-5">
          <div>
            ...
            <p className="section-copy">{operationDetails?.description ?? "Run a geo-io-geojson operation."}</p>
          </div>
